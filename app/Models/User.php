...
class User extends Authenticatable implements BannableContract, Auditable, JWTSubject, MustVerifyEmail, CanResetPassword
{
    ...
    public function isServerBanned(){
        return $this->getServerBans()->count() > 0;
    }

    /**
     *
     * Метод должен возвращать Collection из банов, где бан - объект.
     * Обязательные поля для бана:
     * id - ID бана
     * price - цена разбана для этого бана
     * admin - объект пользователя, выдавшего блокировку
     * Reason - причина бана
     * Time - таймштамп бана
     * Temptime - таймштамп окончания бана или false
     *
     * Если не разбираетесь, обратитесь к автору и сообщите структуру таблицы с банами.
     *
     * @return Collection
     */
    ...
    public function getAllRoles(){
        ...
        $allroles->push(\Backpack\PermissionManager\app\Models\Role::findByName('member', 'web'));

        $this->getActiveGroups()->each(function ($item, $key) use (&$allroles){
            try{
        ...
        return $allroles->merge($this->roles()->get())->unique('id');
    }
