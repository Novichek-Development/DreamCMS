...
class ProfileController extends Controller
{
    ...
    public function refers(Request $request){
        ...
        $refers = $refers->map(function ($refer){
            $refer->votes = \DB::table('top_votes')->where([
                'user_id' => $refer->id
            ])->count();
            $refer->reg_time = Carbon::createFromTimestamp($refer->reg_time)->format('d.m.Y H:m');
            return $refer->only('id', 'login', 'uuid', 'reg_time', 'votes');
        });
        ...
        return [
            ...
            'refers' => $refers,
            'settings' => [
                'balance_percent' => config('settings.refer_profit', 0),
                'tops_percent' => config('settings.refer_profit_tops', 0)
            ]
        ];
    }
    ...
    public function buyKit(Request $request){
        ...
        $kit = Kit::where('name', '=', $kit_name)->first();

        if (!$kit){
            return Response::json([
                'success' => true,
                'message' => 'Кит не найден!'
            ]);
        }
    ...
    public function buygroup(Request $request)
    {
        ...
        if(Auth::user()->realmoney < $group->price){
            return Response::json(array(
                'success' => false,
                'message' => 'Недостаточно ' . ($group->price - Auth::user()->realmoney) . ' рублей для покупки!'
            ));
        }
