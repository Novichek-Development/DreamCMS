            ...
            <div class="window addBalance">
                ...
                <div class="px-3 px-sm-4 px-md-5 pt-3 pt-sm-4 pt-md-5 pb-3 pb-sm-4">
                    ...
                    <div class="bonus mt-2" v-else>
                        После пополнения Вы получите <span id="mainStreams">{{ youGet }}</span> эйвов<span id="bonusStreams"></span>.
                    </div>
                </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            interkassa(){
                api.post('pay/interkassa', {
                    account: this.selectedUser ? this.selectedUser.uuid : this.user.uuid,
                    sum: this.sum
                }).then(response => {
                    if (response.data.url){
                        this.submit({method: 'POST', path: response.data.url, data: response.data.data});
                    }
                });
            },
            freekassa(){
                api.post('pay/freekassa', {
                    account: this.selectedUser ? this.selectedUser.uuid : this.user.uuid,
                    sum: this.sum
                }).then(response => {
                    if (response.data.url){
